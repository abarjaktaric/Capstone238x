import React from 'react';

import '../css/About.css';
//* rubric79 - Used internal css *//

const About = () => {
    return (
        <main>
            {/* rubric63 - About page */}
            <div className="page about-page">
                <div className="about-banner">
                    <div className="container">
                        <div className="banner-container">
                            <p>World Wide Importers</p>
                            <h1>About us</h1>
                        </div>
                    </div>
                </div>
                <div className="about-paragraph">
                    <div className="container">
                        <div className="row">
                            <div className="col">
                                <h2>WorldWideImporters</h2>
                                <p>Aliquam hendrerit eros malesuada varius vehicula. Mauris laoreet, purus et facilisis porttitor, libero ipsum sodales lacus, vel auctor ipsum lectus eu sapien. Nullam sed mauris ut orci dignissim gravida. Cras interdum metus et mauris interdum rhoncus. Sed ipsum nisi, feugiat eget sapien vel, ultrices tincidunt libero. Praesent malesuada felis a urna pulvinar, quis maximus lacus rhoncus. Vivamus bibendum gravida sodales. Nam sagittis porta volutpat. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Aenean ut lorem arcu. Fusce tempor felis ut risus eleifend blandit. Donec tempor quis nunc ut placerat. Pellentesque non luctus eros, sed interdum massa.</p>
                                <p>Nullam quis purus vel libero semper egestas at nec urna. Aliquam maximus magna neque, ut efficitur mauris fringilla feugiat. Sed sit amet imperdiet nisl. Proin accumsan, nibh consectetur lobortis tincidunt, ipsum risus vulputate purus, quis efficitur ex tellus ut nunc. Aliquam aliquet facilisis maximus. Integer malesuada erat eu sem sodales condimentum. Suspendisse eget ante convallis, mollis urna quis, tempus elit. Etiam vel leo elit. Vivamus ut orci vitae nisl condimentum lobortis ac id orci. Integer sed felis elit. Praesent posuere mauris nisl. Curabitur bibendum, eros ut fermentum sodales, velit eros porttitor lacus, et tincidunt nisl mauris at dolor. Nulla at leo vitae quam pulvinar commodo. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Quisque ornare est a mi mattis, ut pretium diam commodo.</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="about-visual">
                    <div className="container">
                        <div className="row">
                            <div className="col center">
                                <h3>Why we are the best</h3>
                            </div>
                        </div>
                        <div className="row">
                            <div className="col">
                                <div className="media">
                                    <div className="media-img">
                                        <img src="img/bicycle.jpg" alt="" />
                                    </div>
                                    <h4>Fast Delivery</h4>
                                </div>
                            </div>
                            <div className="col">
                                <div className="media">
                                    <div className="media-img">
                                        <img src="img/woman.jpg" alt="" />
                                    </div>
                                    <h4>Easy transaction</h4>
                                </div>
                            </div>
                            <div className="col">
                                <div className="media">
                                    <div className="media-img">
                                        <img src="img/money.jpg" alt="" />
                                    </div>
                                    <h4>Safe Shopping</h4>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="about-paragraph">
                    <div className="container">
                        <div className="row">
                            <div className="col">
                                <h3>Our Vision</h3>
                                <p>Aliquam hendrerit eros malesuada varius vehicula. Mauris laoreet, purus et facilisis porttitor, libero ipsum sodales lacus, vel auctor ipsum lectus eu sapien. Nullam sed mauris ut orci dignissim gravida. Cras interdum metus et mauris interdum rhoncus. Sed ipsum nisi, feugiat eget sapien vel, ultrices tincidunt libero. Praesent malesuada felis a urna pulvinar, quis maximus lacus rhoncus. Vivamus bibendum gravida sodales. Nam sagittis porta volutpat. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Aenean ut lorem arcu. Fusce tempor felis ut risus eleifend blandit. Donec tempor quis nunc ut placerat. Pellentesque non luctus eros, sed interdum massa.</p>
                                <h3>Other information</h3>
                                <p>Aliquam hendrerit eros malesuada varius vehicula. Mauris laoreet, purus et facilisis porttitor, libero ipsum sodales lacus, vel auctor ipsum lectus eu sapien. Nullam sed mauris ut orci dignissim gravida. Cras interdum metus et mauris interdum rhoncus. Sed ipsum nisi, feugiat eget sapien vel, ultrices tincidunt libero. Praesent malesuada felis a urna pulvinar, quis maximus lacus rhoncus. Vivamus bibendum gravida sodales. Nam sagittis porta volutpat. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Aenean ut lorem arcu. Fusce tempor felis ut risus eleifend blandit. Donec tempor quis nunc ut placerat. Pellentesque non luctus eros, sed interdum massa.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </main>
    );
}

export default About;